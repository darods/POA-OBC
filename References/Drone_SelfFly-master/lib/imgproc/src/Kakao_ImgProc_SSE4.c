...
/*----------------------------------------------------------------------------------------
 Static Function
 ----------------------------------------------------------------------------------------*/
static void _GetMean4x4_SSE4(IN const UINT8 *pSrc, IN const INT32 nSrcStride, OUT UINT32 *pSum);
static void _GetMean8x8_SSE4(IN const UINT8 *pSrc, IN const INT32 nSrcStride, OUT UINT32 *pSum);
static void _GetMean16x16_SSE4(IN const UINT8 *pSrc, IN const INT32 nSrcStride, OUT UINT32 *pSum);
static void _GetMean_SSE4(IN const UINT8 *pSrc, IN const INT32 nSrcStride, IN const INT32 nWidth, IN const INT32 nHeight,
                             OUT UINT32 *pSum, fpgetNxMMean fpMean, IN const INT32 nBlockSize);

static void _GetVar4x4_SSE4(IN const UINT8 *pSrc, IN const INT32 nSrcStride, OUT UINT32 *pSqSum, OUT UINT32 *pSum);
static void _GetVar8x8_SSE4(IN const UINT8 *pSrc, IN const INT32 nSrcStride, OUT UINT32 *pSqSum, OUT UINT32 *pSum);
static void _GetVar16x16_SSE4(IN const UINT8 *pSrc, IN const INT32 nSrcStride, OUT UINT32 *pSqSum, OUT UINT32 *pSum);
static void _GetDiffVar4x4_SSE4(IN const UINT8 *pSrc0, IN const INT32 nSrcStride0,
                             IN const UINT8 *pSrc1, IN const INT32 nSrcStride1, OUT UINT32 *pSqSum, OUT INT32 *pSum);
static void _GetDiffVar8x8_SSE4(IN const UINT8 *pSrc0, IN const INT32 nSrcStride0,
                             IN const UINT8 *pSrc1, IN const INT32 nSrcStride1, OUT UINT32 *pSqSum, OUT INT32 *pSum);
static void _GetDiffVar16x16_SSE4(IN const UINT8 *pSrc0, IN const INT32 nSrcStride0,
                             IN const UINT8 *pSrc1, IN const INT32 nSrcStride1, OUT UINT32 *pSqSum, OUT INT32 *pSum);
static void _GetVariance_SSE4(IN const UINT8 *pSrc0, IN const INT32 nSrcStride0, IN const INT32 nWidth, IN const INT32 nHeight,
                             OUT UINT32 *pSqSum, OUT UINT32 *pSum, fpgetNxMVar fpVar, IN const INT32 nBlockSize);
static void _GetDiffVariance_SSE4(IN const UINT8 *pSrc0, IN const INT32 nSrcStride0, IN const UINT8 *pSrc1, IN const INT32 nSrcStride1,
                             IN const INT32 nWidth, IN const INT32 nHeight, OUT UINT32 *pSqSum, OUT INT32 *pSum, fpgetNxMDiffVar fpVar, IN const INT32 nBlockSize);
...
/*----------------------------------------------------------------------------------------
 ...
 ----------------------------------------------------------------------------------------*/
#if ENABLE_ISET(CPU_INTEL_IA32 | ISET_SSE4) | ENABLE_ISET(CPU_INTEL_IA64 | ISET_SSE4)
void OneRow_Copy_SSE4(OUT UINT8 *pDst, IN const UINT8 *pSrc, IN const INT32 nWidth)
{
    MEMCPY(pDst, pSrc, nWidth);
}


void OneRow_InterleaveUV_SSE4(IN const UINT8 *pSrcUV, OUT UINT8 *pDstU, OUT UINT8 *pDstV, IN const INT32 nWidth)
{
    INT32               j = 0;
...
void OneRow_Subsample_Even_SSE4(IN const UINT8 *pSrc, OUT UINT8 *pDst, IN const INT32 nWidth)
{
    INT32               j = 0;
    const INT32         nScaledWidth = (nWidth >> 3);
...
void OneRow_Subsample_Odd_SSE4(IN const UINT8 *pSrc, OUT UINT8 *pDst, IN const INT32 nWidth)
{
    INT32               j = 0;
    const INT32         nScaledWidth = (nWidth >> 3);
...
void OneRow_Subsample_1_4_SSE4(IN const UINT8 *pSrc, OUT UINT8 *pDst, IN const INT32 nWidth)
{
    INT32               j = 0;
    const INT32         nScaledWidth = (nWidth >> 3);
...
void OneRow_ScaleBilinear_SSE4(OUT UINT8* pDst,
...
void OneCol_ScaleBilinear_SSE4(OUT UINT8* pDst, IN const UINT8* pSrc,
...
void OneRow_MeanBlur_SSE4(IN const UINT8 *pSrc, OUT UINT8 *pDst, IN const INT32 nWidth, IN const INT32 nSrcStride, IN const INT32 nDstStride)
{
    INT32                   i = 0;
...
void OneRow_MeanBlur_Hor_SSE4(IN const UINT8 *pSrc, OUT UINT8 *pDst, IN const INT32 nWidth)
{
    INT32                   i = 0;
...
void OneRow_MeanBlur_Ver_SSE4(IN const UINT8 *pSrc, OUT UINT8 *pDst, IN const INT32 nWidth, IN const INT32 nSrcStride, IN const INT32 nDstStride)
{
    INT32                   i = 0;
...
void OneRow_Mosaic4_SSE4(IN const UINT8 *pSrc, OUT UINT8 *pDst, IN const INT32 nWidth, IN const INT32 nSrcStride, IN const INT32 nDstStride, IN const RectImgAddr *pBaseRectAddr)
{
    INT32                   i = 0, j = 0;
...
void OneRow_Mosaic8_SSE4(IN const UINT8 *pSrc, OUT UINT8 *pDst, IN const INT32 nWidth, IN const INT32 nSrcStride, IN const INT32 nDstStride, IN const RectImgAddr *pBaseRectAddr)
{
    INT32                   i = 0, j = 0;
...
void OneRow_Mosaic16_SSE4(IN const UINT8 *pSrc, OUT UINT8 *pDst, IN const INT32 nWidth, IN const INT32 nSrcStride, IN const INT32 nDstStride, IN const RectImgAddr *pBaseRectAddr)
{
    INT32                   i = 0, j = 0;
...
void OneRow_Inverse_SSE4(IN const UINT8 *pSrc, OUT UINT8 *pDst, IN const INT32 nWidth)
{
    INT32                   i = 0;
    ...
    {
        *(__m128i *)pDst = _mm_subs_epu8(nPixVal255, *(__m128i *)pSrc);
...
void OneRow_Clip_SSE4(IN const UINT8 *pSrc, OUT UINT8 *pDst, IN const INT32 nWidth, IN const INT32 nMinClipVal, IN const INT32 nMaxClipVal)
{
    INT32                   i = 0;
...
void OneRow_DiffImg_SSE4(IN const UINT8 *pSrc0, IN const UINT8 *pSrc1, OUT UINT8 *pDst, IN const INT32 nWidth)
{
    INT32                   i = 0;
    const INT32             nLoopNum = nWidth >> 4;
    const INT32             nOffset = 16;
...
void OneRow_Multiply_SSE4(IN const UINT8 *pSrc0, IN const UINT8 *pSrc1, OUT UINT8 *pDst, IN const INT32 nWidth)
{
    INT32                   i = 0;
    const INT32             nLoopNum = nWidth >> 4;
    const INT32             nOffset = 16;
...
void OneRow_ContrastShift_SSE4(IN const UINT8 *pSrc0, IN const UINT8 *pSrc1, OUT UINT8 *pDst, IN const INT32 nWidth)
{
    INT32                   i = 0;
    const INT32             nLoopNum = nWidth >> 4;
    const INT32             nOffset = 16;
...
void OneRow_PropMix_SSE4(IN const UINT8 *pSrc0, IN const UINT8 *pSrc1, IN const F32 *pCoeffi, OUT UINT8 *pDst, IN const INT32 nWidth)
{
    INT32                   i = 0;
    const INT32             nLoopNum = nWidth >> 4;
    const INT32             nOffset = 16;
    ...
    for(i=0 ; i<nLoopNum ; i++)
    {
        ...
        nSrc0iL16b = _mm_unpacklo_epi8(*(__m128i *)pSrc0, nZero);
        nSrc0iH16b = _mm_unpackhi_epi8(*(__m128i *)pSrc0, nZero);
        nSrc1iL16b = _mm_unpacklo_epi8(*(__m128i *)pSrc1, nZero);
        nSrc1iH16b = _mm_unpackhi_epi8(*(__m128i *)pSrc1, nZero);
...
void OneRow_BlurNoiseReduction_SSE4(IN const UINT8 *pSrc, OUT UINT8 *pDst, IN const INT32 nWidth, IN const INT32 nSrcStride, IN const INT32 nDstStride)
{
    INT32                   i = 0;
    UINT8                   *pSrcPrev = NULL, *pSrcNext0 = NULL, *pSrcNext1 = NULL, *pSrcNext2 = NULL;
    UINT8                   *pDst0 = NULL, *pDst1 = NULL, *pDst2 = NULL;
...
static void _GetVariance_SSE4(IN const UINT8 *pSrc, IN const INT32 nSrcStride, IN const INT32 nWidth, IN const INT32 nHeight,
                              OUT UINT32 *pSqSum, OUT UINT32 *pSum, fpgetNxMVar fpVar, IN const INT32 nBlockSize)
{
    INT32                   i = 0, j = 0;
...
F32 Mean_MxN_SSE4(IN const UINT8 *pSrc, IN const INT32 nSrcStride, IN const INT32 nSizeX, IN const INT32 nSizeY, OUT UINT32 *pSum)
{
    const INT32             nPatchSize = nSizeX * nSizeY;
    UINT32                  nLocalSum = 0;
    
    if(16 == nPatchSize)
        _GetMean4x4_SSE4(pSrc, nSrcStride, &nLocalSum);
    else if(32 == nPatchSize)
        _GetMean_SSE4(pSrc, nSrcStride, nSizeX, nSizeY, &nLocalSum, _GetMean4x4_SSE4, 4);
    else if(64 == nPatchSize)
        _GetMean8x8_SSE4(pSrc, nSrcStride, &nLocalSum);
    else if(128 == nPatchSize)
        _GetMean_SSE4(pSrc, nSrcStride, nSizeX, nSizeY, &nLocalSum, _GetMean8x8_SSE4, 8);
    else if(256 == nPatchSize)
        _GetMean16x16_SSE4(pSrc, nSrcStride, &nLocalSum);
...
F32 Variance_MxN_SSE4(IN const UINT8 *pSrc, IN const INT32 nSrcStride, IN const INT32 nSizeX, IN const INT32 nSizeY, OUT UINT32 *pSqSum, OUT UINT32 *pSum)
{
    const INT32             nPatchSize = nSizeX * nSizeY;
    UINT32                  nLocalSum = 0;
    
    if(16 == nPatchSize)
        _GetVar4x4_SSE4(pSrc, nSrcStride, pSqSum, &nLocalSum);
    else if(32 == nPatchSize)
        _GetVariance_SSE4(pSrc, nSrcStride, nSizeX, nSizeY, pSqSum, &nLocalSum, _GetVar4x4_SSE4, 4);
    else if(64 == nPatchSize)
        _GetVar8x8_SSE4(pSrc, nSrcStride, pSqSum, &nLocalSum);
    else if(128 == nPatchSize)
        _GetVariance_SSE4(pSrc, nSrcStride, nSizeX, nSizeY, pSqSum, &nLocalSum, _GetVar8x8_SSE4, 8);
    else if(256 == nPatchSize)
        _GetVar16x16_SSE4(pSrc, nSrcStride, pSqSum, &nLocalSum);
...
F32 DiffVariance_MxN_SSE4(IN const UINT8 *pSrc0, IN const INT32 nSrcStride0, IN const UINT8 *pSrc1, IN const INT32 nSrcStride1,
                          IN const INT32 nSizeX, IN const INT32 nSizeY, OUT UINT32 *pSqSum, OUT INT32 *pSum)
{
    ...
    if(16 == nPatchSize)
        _GetDiffVar4x4_SSE4(pSrc0, nSrcStride0, pSrc1, nSrcStride1, pSqSum, &nLocalSum);
    else if(32 == nPatchSize)
        _GetDiffVariance_SSE4(pSrc0, nSrcStride0, pSrc1, nSrcStride1, nSizeX, nSizeY, pSqSum, &nLocalSum, _GetDiffVar4x4_SSE4, 4);
    else if(64 == nPatchSize)
        _GetDiffVar8x8_SSE4(pSrc0, nSrcStride0, pSrc1, nSrcStride1, pSqSum, &nLocalSum);
    else if(128 == nPatchSize)
        _GetDiffVariance_SSE4(pSrc0, nSrcStride0, pSrc1, nSrcStride1, nSizeX, nSizeY, pSqSum, &nLocalSum, _GetDiffVar8x8_SSE4, 8);
    else if(256 == nPatchSize)
        _GetDiffVar16x16_SSE4(pSrc0, nSrcStride0, pSrc1, nSrcStride1, pSqSum, &nLocalSum);
