...
void setup(){
  ...
  DDRD |= B10000000;                                          //Configure digital poort 7 as output.
  ...
  battery_voltage = (analogRead(3) + 65) * 1.2317;
...
void loop(){
  ...
  //0.09853 = 0.08 * 1.2317.
  battery_voltage = battery_voltage * 0.92 + (analogRead(3) + 65) * 0.09853;
  ...
  if (start == 2){                                                          //The motors are started.
    ...
    esc_1 = throttle - pid_output_pitch - pid_output_roll + pid_output_yaw; //Calculate the pulse for esc 1 (front-left - CW)
    esc_2 = throttle - pid_output_pitch + pid_output_roll - pid_output_yaw; //Calculate the pulse for esc 2 (front-right - CCW)
    esc_3 = throttle + pid_output_pitch + pid_output_roll + pid_output_yaw; //Calculate the pulse for esc 3 (rear-right - CW)
    esc_4 = throttle + pid_output_pitch - pid_output_roll - pid_output_yaw; //Calculate the pulse for esc 4 (rear-left - CCW)
  ...
  while(PORTB & B00001111){                                        //Execute the loop until digital port 8 to 11 is low
    ...
    if(timer_channel_4 <= esc_loop_timer)PORTB &= B11110111; //When the delay time is expired, digital port 11 is set low
        ...
        nEndTime = micros();
    Serial.print(" Loop Duration: "); Serial.println((nEndTime - nStartTime0)/1000);
